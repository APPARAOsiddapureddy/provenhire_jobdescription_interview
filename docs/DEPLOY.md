# Deploying Proven Hire Job Description Interview
...
`python -m proven_hire_agent.worker` runs the **worker**. The worker imports
...
- **web** — `apps/web/Dockerfile`, multi-stage, Next.js **standalone** output
  ...
  root** (pnpm + Turborepo monorepo); `@proven-hire/shared` is built first via
  `pnpm build --filter @proven-hire/web...`. The runner is a slim
...
`buildCommand: pnpm build --filter @proven-hire/web...` (the trailing `...`
builds the `@proven-hire/shared` workspace dependency first), with
...
- **Agent** — `apps/agent/src/proven_hire_agent/core/observability.py` exposes
