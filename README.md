...
<img src="assets/logo.svg" width="132" alt="Proven Hire Job Description Interview" />

# Proven Hire Job Description Interview: Voice-First, Multilingual AI Mock Interviewer
...
[Quickstart](#quickstart) · [Why](#why-proven-hire-job-description-interview) · [Features](#features) · [Architecture](#architecture) · [Community](#community) · [Contributing](#contributing)
...
![Proven Hire Job Description Interview demo — a live voice interview with streaming captions, then the scored report](assets/demo.gif)
...
Proven Hire Job Description Interview closes the **prep ⇄ interview ⇄ feedback** loop: heavy reasoning runs *before* the call (read your CV + the JD, research the company, build an adaptive question plan), a lean real-time voice loop runs the interview, then strong models score it and route you into a study coach for your weak areas.
...
> `pnpm build` must run before `pnpm proven-hire init` — the CLI is built into `cli/dist/`.
...
```bash
pnpm proven-hire init    # or: cp .env.example .env  (keys are optional — see note)
...
## Why Proven Hire Job Description Interview

Practicing in your head (or in a text chat) isn't how interviews work. Proven Hire Job Description Interview is **voice-first** — you answer out loud, in real time, like the real thing — and built to be **owned, not rented**:
...
- **Community playbook library** — question-bank packs in [`skills/`](skills/) (versioned Markdown + YAML) are retrieved by role/level and injected into the Question Planner: packs the community writes get asked in real interviews. Validate yours with `pnpm proven-hire skills lint`.
...
## Using Proven Hire Job Description Interview
...
If Proven Hire Job Description Interview helps your work, please cite it. Full metadata is in [`CITATION.cff`](CITATION.cff).

```bibtex
@software{proven-hire2026,
  title  = {Proven Hire Job Description Interview: Voice-First, Multilingual AI Mock Interviewer},
  author = {The Proven Hire Job Description Interview contributors},
  year   = {2026},
  license = {Apache-2.0},
...
[back to top](#proven-hire-job-description-interview-voice-first-multilingual-ai-mock-interviewer)
