...
import { LANGUAGES } from "@proven-hire/shared";
...
function nextSteps(values: Values): string {
  ...
  lines.push(
    "",
    "Re-run `proven-hire init` anytime — existing values are offered as defaults.",
  );
  return lines.join("\n");
}
...
function runNonInteractive(
  ...
  if (existsSync(target) && !force) {
    console.error(
      ".env already exists — re-run with --force to re-sync the local-dev " +
        "copies (apps/agent/.env, apps/web/.env.local), or run `proven-hire " +
  ...
  if (!existsSync(target)) {
    ...
    console.log(
      `✓ Wrote ${target} from .env.example (non-interactive). Fill in keys, or ` +
        "run `proven-hire init` in a terminal for the guided wizard.",
    );
  } else {
  ...
  console.log(
    `\nProven Hire Job Description Interview supports ${LANGUAGES.length} languages (English-first).`,
  );
}
...
export async function runInit(args: string[]): Promise<void> {
  ...
  intro("Proven Hire Job Description Interview setup");
