/**
 * `proven-hire avatars pull`            — fetch + SHA-256-verify accepted packs
 * `proven-hire avatars verify <files…>` — contributor pre-flight on local files
 *
...
async function loadManifest(): Promise<AvatarManifest> {
  if (!existsSync(MANIFEST_PATH)) {
    throw new Error(
      `${MANIFEST_PATH} not found — run from the repo root (pnpm proven-hire avatars pull).`,
    );
  }
...
export async function runAvatars(args: string[]): Promise<void> {
  ...
  console.error("Usage: proven-hire avatars <pull | verify <files…>>");
  process.exitCode = 1;
}
