...
The Proven Hire Job Description Interview **knowledge service** that powers the Prep Coach. It keeps one
...
`@proven-hire/shared` — the KB wire models are mirrored locally in
...
```bash
docker build -t proven-hire-lightrag services/lightrag
docker run -p 9621:9621 proven-hire-lightrag   # NaiveRAG backend
```
